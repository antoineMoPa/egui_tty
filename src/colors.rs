//! The colors a shell draws with, per scheme.
//!
//! Both schemes are set as explicit colors, including the dark one that started as the
//! emulator's own. Handing the emulator `None` to mean "back to your defaults" does not put
//! them back, so a shell that had been through the light scheme once stayed light — which
//! against a dark background is text the color of its own background.
...
use crate::{ColorScheme, Error, Result};

/// The light scheme's own colors, dark enough to read on paper. `white` is a grey rather than
/// white for the same reason, and `bright white` is the darkest of all: programs reach for it
/// to emphasise, which on a light background means going darker.
...
/// changed it. This is what [`ColorScheme::Dark`] is — Ghostty's own colors — kept so they
/// can be put back exactly rather than asked for again.
...
/// Point the emulator at the scheme asked for.
pub(crate) fn apply(
    ...
    let colors = match scheme {
        ColorScheme::Dark => emulator.clone(),
        ColorScheme::Light => light_from(emulator),
    };

    terminal
        .set_default_fg_color(Some(colors.foreground))
        .and_then(|terminal| terminal.set_default_bg_color(Some(colors.background)))
        .and_then(|terminal| terminal.set_default_cursor_color(Some(colors.cursor)))
        .and_then(|terminal| terminal.set_default_color_palette(Some(colors.palette)))
        .map_err(|error| Error::context("failed to set the terminal's colors", error))?;
    Ok(())
}
...
mod tests {
    ...
    /// What the widget would actually paint with, which is not the same question as what the
    ...
    fn every_light_color_is_readable_on_the_light_background() {
        ...
        apply(&mut terminal, &emulator, ColorScheme::Light).expect("expected the light colors");
    ...
    /// Light and then dark again must not leave a shell drawing its text in the same color as
    /// its background: the emulator does not take `None` to mean "back to your own colors".
    #[test]
    fn a_round_trip_through_light_puts_the_dark_colors_back_exactly() {
        ...
        assert!(
            ...
            "the light scheme really did change something"
        );

        apply(&mut terminal, &emulator, ColorScheme::Dark).expect("expected the dark colors back");
