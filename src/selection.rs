...
impl Pointer {
    pub(crate) fn new() -> Result<Self> {
        Ok(Self {
            gesture: Gesture::new()
                .map_err(|error| Error::context("failed to create a selection gesture", error))?,
            press: PressEvent::new()
                .map_err(|error| Error::context("failed to create a press event", error))?,
            drag: DragEvent::new()
                .map_err(|error| Error::context("failed to create a drag event", error))?,
    ...
    pub(crate) fn press(
        ...
        self.press
            ...
            .map_err(|error| Error::context("failed to describe the press", error))?;

        let selection = self
            .press
            .apply(&mut self.gesture, terminal, grid_ref)
            .map_err(|error| Error::context("failed to apply the press", error))?;
        terminal
    ...
    pub(crate) fn drag(
        ...
        self.drag
            ...
            .map_err(|error| Error::context("failed to describe the drag", error))?;

        let selection = self
            .drag
            .apply(&mut self.gesture, terminal, grid_ref, geometry)
            .map_err(|error| Error::context("failed to apply the drag", error))?;
    ...
    pub(crate) fn release(&mut self, terminal: &Terminal<'_, '_>) -> Result<()> {
        self.dragging = false;
        self.release
            .apply(&mut self.gesture, terminal, None)
            .map_err(|error| Error::context("failed to apply the release", error))
    }
...
fn grid_ref_at<'t>(
    ...
    terminal
        ...
        .map_err(|error| Error::context("failed to read the cell under the pointer", error))
}
...
/// stripped either way.
pub(crate) fn encode_paste(terminal: &Terminal<'_, '_>, text: &str) -> Result<Vec<u8>> {
    ...
    match paste::encode(&mut data, bracketed, &mut out) {
        ...
        Err(libghostty_vt::error::Error::OutOfSpace { required }) => {
            ...
            let written = paste::encode(&mut data, bracketed, &mut out)
                .map_err(|error| Error::context("failed to encode the paste", error))?;
            out.truncate(written);
            Ok(out)
        }
        Err(error) => Err(Error::context("failed to encode the paste", error)),
    }
}
